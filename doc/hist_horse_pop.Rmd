...
```{r plot horses, fig.width=10, fig.height=3, fig.cap="Figure 1. Historical number of per province in Canada", out.width = '100%'}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1. This is a summary of the historical population of horses data set")
```
From above, we know that the maximum value in `r max_horses$Province[1]` is `r max_horses$Maximum[1]` and the minimum value in `r max_horses$Province[1]` is `r max_horses$Minimum[1]`

Below we zoom in and look at the province of `r max_horses$Province[1]`:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Figure 2. Historical number of horses per province in Canada", out.width='50%'}
